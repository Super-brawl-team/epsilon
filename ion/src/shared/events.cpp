...
Event::Event(Keyboard::Key key, bool shift, bool alpha, bool lock) {
  ...
  // This can be a bit more complicated than it seems since we want to fall
  // back: for example, alpha-up is just plain up. Fallback order : shift-X -> X
  // alpha-X -> X
  ...
  while (true) {
    ...
    while ((state = Keyboard::popState()) != Keyboard::State(-1)) {
      ...
      if (keysSeenTransitioningFromUpToDown != 0) {
        ...
        /* The key that triggered the event corresponds to the first non-zero
         * bit in "match". This is a rather simple logic operation for the which
    ...
    bool isRepeatableEvent =
        ...
        m_lastEventAlpha ==
            (m_currentKeyboardState.keyDown(Keyboard::Key::Alpha) || lock);
