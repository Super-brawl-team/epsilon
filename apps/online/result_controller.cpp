...
void ResultController::viewWillAppear() {
  ...
  m_cell.label()->setMessage(App::GetOnlineData()->labelForParameter(
      App::GetOnlineData()->getUnknown()));
  m_cell.subLabel()->setMessage(App::GetOnlineData()->sublabelForParameter(
      App::GetOnlineData()->getUnknown()));
  double value = App::GetOnlineData()->computeUnknownValue();
...
const char* ResultController::title() const {
  ...
  uint8_t doubleParameters = App::GetOnlineData()->numberOfDoubleValues();
  ...
  for (uint8_t param = 0; param < doubleParameters; param++) {
    ...
    int parameterLength = Poincare::Print::UnsafeCustomPrintf(
        ...
        I18n::translate(App::GetOnlineData()->labelForParameter(param)),
        App::GetOnlineData()->getValue(param), printFloatMode, precision);
