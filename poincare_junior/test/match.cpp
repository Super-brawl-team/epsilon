...
void testContext() {
  using enum Placeholder;
  Context ctx;
  constexpr Tree t = Add(2_n, 1_n);
  ctx[A] = t;
  constexpr Tree structure = Mult(5_n, Add(Tree(A), Tree(A)));
...
void testRewrite() {
  ...
  constexpr Tree s = Mult(2_n, Tree(A));
  ...
  EditionReference result2 = ref.rewrite(Add(a, a), Mult(2_n, a));
  result2.log();
}
