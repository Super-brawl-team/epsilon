...
QUIZ_CASE(ion_utf8_helper_not_code_point_search) {
  ...
  s = "π234";
  assert_not_code_point_searched_is(
      s, UCodePointGreekSmallLetterPi, false, nullptr,
      s + UTF8Decoder::CharSizeOfCodePoint(UCodePointGreekSmallLetterPi));
  // Going left
  s = "aaaaa";
  assert_not_code_point_searched_is(s, 'a', true, s + 5, s);
  assert_not_code_point_searched_is(s, 'b', true, s + 5, s + 4);
  s = "234π";
  assert_not_code_point_searched_is(s, UCodePointGreekSmallLetterPi, true,
                                    s + 5, s + 2);
}
...
QUIZ_CASE(ion_utf8_copy_and_remove_code_point) {
  ...
  CodePoint c2[] = {'2'};
  result = "1345";
  assert_copy_and_remove_code_points_gives(buffer, bufferSize, s, c2, 1,
                                           result);

  s = "2123224252";
  CodePoint c3[] = {'2'};
  result = "1345";
  assert_copy_and_remove_code_points_gives(buffer, bufferSize, s, c3, 1,
                                           result);

  s = "12345";
  CodePoint c4[] = {'6'};
  result = "12345";
  assert_copy_and_remove_code_points_gives(buffer, bufferSize, s, c4, 1,
                                           result);

  s = "12ᴇ4";
  CodePoint c5[] = {UCodePointLatinLetterSmallCapitalE};
  result = "124";
  assert_copy_and_remove_code_points_gives(buffer, bufferSize, s, c5, 1,
                                           result);

  s = "12ᴇᴇᴇ4";
  CodePoint c6[] = {UCodePointLatinLetterSmallCapitalE};
  result = "124";
  assert_copy_and_remove_code_points_gives(buffer, bufferSize, s, c6, 1,
                                           result);
  ...
  result = "1233";
  assert_copy_and_remove_code_points_gives(buffer, bufferSize, s, c9, 2,
                                           result);
}
...
QUIZ_CASE(ion_utf8_helper_previous_code_point_is) {
  ...
  s = "1234∑";
  quiz_assert(UTF8Helper::PreviousCodePointIs(s, s + strlen(s),
...
void assert_string_remove_previous_glyph_gives(const char *text, char *location,
  ...
  CodePoint c = 0;
  quiz_assert(UTF8Helper::RemovePreviousGlyph(text, location, &c) ==
              returnedResult);
...
QUIZ_CASE(ion_utf8_helper_beginning_of_word) {
  ...
  assert_beginning_of_word_is(test_sentence, test_sentence + 5,
                              test_sentence + 3);
  assert_beginning_of_word_is(test_sentence, test_sentence + 8,
                              test_sentence + 8);
}
