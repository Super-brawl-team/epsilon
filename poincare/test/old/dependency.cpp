...
QUIZ_CASE(poincare_dependency_function) {
  ...
  assert_parsed_expression_simplify_to("g(x)", "dep(3,{x})");
  assert_parsed_expression_simplify_to("g(x+a+3+4)", "dep(3,{x})");
...
QUIZ_CASE(poincare_dependency_derivative) {
  ...
  assert_parsed_expression_simplify_to("diff(2x,x,y)", "dep(2,{y})");
  assert_parsed_expression_simplify_to("diff(f(x),x,a)", "2×a");
  Ion::Storage::FileSystem::sharedFileSystem->recordNamed("f.func").destroy();

  assert_reduce_and_store("3→f(x)");
  assert_parsed_expression_simplify_to("diff(cos(x)+f(y),x,0)", "0,{y})");
  assert_parsed_expression_simplify_to("diff(cos(x)+f(y),x,x)",
                                       "dep(-sin(x),{y,cos(x)})");
...
QUIZ_CASE(poincare_dependency_parametered_expression) {
  ...
  assert_reduce_and_store("1→f(x)");
  assert_parsed_expression_simplify_to("f(a)", "dep(1,{a})");
...
QUIZ_CASE(poincare_dependency_power) {
  assert_parsed_expression_simplify_to("1/(1/x)", "dep(x,{1/x})");
...
QUIZ_CASE(poincare_dependency_multiplication) {
  assert_parsed_expression_simplify_to("ln(x)-ln(x)", "dep(0,{ln(x)})");
  ...
  assert_parsed_expression_simplify_to("0x^arcsin(π)", "dep(0,{x})");
  assert_parsed_expression_simplify_to("0x^arcsin(π)", Undefined::Name(),
