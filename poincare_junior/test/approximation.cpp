...
QUIZ_CASE(pcj_approximation_replace) {
  EditionReference ref1(KAdd(1_e, 2_e, 10.5_fe));
  quiz_assert(Approximation::ApproximateAndReplaceEveryScalar(ref1));
  assert_trees_are_equal(ref1, 13.5_fe);

  EditionReference ref2(
      KMult(2.0_fe, KDiv("x"_e, KAdd(1_e, 2.0_fe)), KAdd(1_e, 2_e, 10.5_fe)));
  quiz_assert(Approximation::ApproximateAndReplaceEveryScalar(ref2));
  assert_trees_are_equal(ref2, KMult(2.0_fe, KDiv("x"_e, 3.0_fe), 13.5_fe));
}
